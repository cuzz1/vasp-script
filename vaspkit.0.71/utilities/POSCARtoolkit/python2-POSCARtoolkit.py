...
#version 1.3
#时间 2018.10.17 
#新增功能：通过选项-f 后选择 all  可以固定所有层；通过flag参数控制功能使用 -h 帮助  -i 指定输入文件  
...
def interpret_car(lines):
    ...
    dict_car3 = dict(zip(dict_car2.values(), dict_car2.keys()))   
...
def convert(start_num,**kwargs):
    ...
    if options.fixed and not  options.selected:
        ...
        fixedlayer = raw_input("    Found %d layers, choose how many layers to be fixed------>" %len(layerscount))
        if fixedlayer=="all":
        	fixedlayer = len(layerscount)
        elif fixedlayer.isdigit() and int(fixedlayer) <= len(layerscount):
        	fixedlayer=int(fixedlayer)
        else:
        	print(u"Error layers input!")
